 ...
 * 
 * K is how many hash functions will be used by this Bloom Filter. It is optimal
 * value can be calculated based on bitArraySize and expected element amount
 *
 */
public class ImplementBloomFilter {
    /**
     * A simple Bloom Filter (see http://en.wikipedia.org/wiki/Bloom_filter)
     * that uses java.util.Random as a primitive hash function, and which
     * implements Java's Set interface for convenience.
     * 
	...
	/**
	 * Construct a SimpleBloomFilter. You must specify the number of bits in
	 * the Bloom Filter, and also you should specify the number of items you
	 * expect to add. The latter is used to choose some optimal internal
	 * values to minimize the false-positive rate (which can be estimated
	 * with expectedFalsePositiveRate()).
	 * 
	 * @param bitArraySize
	 *            The number of bits in the bit array (often called 'm' in
	...
	/**
	 * Calculates the approximate probability of the contains() method
	 * returning true for an object that had not previously been inserted
	 * into the bloom filter. This is known as the
	 ...
	 *         Filter, true indicates that it probably was. The probability
	 *         can be estimated using the expectedFalsePositiveProbability()
	 *         method.
	 */
