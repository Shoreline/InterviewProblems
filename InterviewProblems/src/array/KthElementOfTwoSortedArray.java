...
/*
 * Kth element -> the Kth smallest element
 * 
 * The tricky part is to distinguish how to handle index and size/length
 */
public class KthElementOfTwoSortedArray {
    /*
     * The best solution, simple and true O(log(m+n)) speed.
     * http://www.acmerblog.com/leetcode-median-of-two-sorted-arrays-5330.html
     */
    class Solution {
	/*
	 * The k-th element of an sorted array C is C[k-1]; k start from 1.
	 * Variables m and n are two pointers.
	 * 
	 * Main idea: exclude a certain fore part of one of the input arrays
	 * each time.
	 * 
	 ...
	 * 
	 * 2) k = 1. In this case we just need to find out the smallest element
	 * of input arrays
	 * 
	 * Each iteration, consider the first k elements of the combined array.
	 * Assume among these k elements, there are pa elements from A[] and pb
	 * from B[].
	 * 
	 * So k = pa + pb (1)
	 * 
	 * The best way is to set pa = pb = k/2, but sometimes A.length or
	 * B.length is smaller than k/2. To avoid unnecessary trouble, let the
	 * length of B[] always <= length of A[].
	 * 
	 * So set pb = min(k/2, B.length);
	 * 
	 * The indices of pa elements in A[] is A[0],...A[pa-1], similar for pb
	 * elments in B[].
	 * 
	 ...
	 * 
	 * If A[pa-1]>B[pb-1], then among these k elements (pa+pb=k) there are
	 * at least 1 element bigger than B[pb-1] (which is A[pa-1]).
	 * 
	 * So there are at most k-1 elements <=B[pb-1], means B[pb-1] is at most
	 * the (k-1)th element in combined array
	 * 
	 * Thus, we can exclude B[0]~B[pb-1], they are impossible to be the k-th
	 * element
	 * 
	 */
	public int findTheKthSmallestElementInTwoSortedArray(int A[], int B[], int k) {
	    return getK(A, 0, B, 0, k);
	}

	private int getK(int A[], int m, int B[], int n, int k) {
	    int sizeA = A.length - m;
	    int sizeB = B.length - n;

	    // let sizeA always >= sizeB
	    if (sizeA < sizeB) {
		return getK(B, n, A, m, k);
	    }

	    if (sizeB == 0) {
		return A[m + k - 1];
	    } else if (k == 1) { // necessary! otherwise error
		...
		return getK(A, m, B, n + pb, k - pb);
	    } else if (A[m + pa - 1] < B[n + pb - 1]) {
		return getK(A, m + pa, B, n, k - pa);
	    } else {
		return A[m + pa - 1];
	    }
	}
	 ...
	 */
	private static double getMedian(int A[], int m1, int m2, int B[], int n1, int n2, int k) {
		...
		if (k > (m2 - m1) / 2 + (n2 - n1) / 2) {
		    return getMedian(A, (m2 - m1) / 2 + 1, m2, B, n1, n2, k - (m2 - m1) / 2 - 1);
		} else {
     ...
     * 
     * The idea is that let i and j elements in A[] and B[] are the first k
     * elements Randomly set i, so j=k-i; (boundary conditions shall be handled
     * beforehand) If A[i]>=B[j-1] and B[j]>=A[i-1] then we find the solution.
     * Otherwise move i.
     */
